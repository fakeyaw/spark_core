...
monoString *CreateMonoString(const char *str, int *hu1, int *hu2) {
    while(mono_address == 0){
        LOGD("ЕБЛАН ОБНОВИ МОНОСТРИНГ");
    }
    monoString *(*String_CreateString)(void *instance, const char *str, int *hu1, int *hu2) = (monoString *(*)(void *,
                                                                                           const char *, int *, int *)) getAdresss(mono_address);
    return String_CreateString(nullptr, str, hu1, hu2);
}
template <typename T>
struct monoArray
{
...
#include <dlfcn.h>
//Всякие дл опены хуёпины для хэндлов нам нахуй не сдались
//Тупо если нужна либка берем с цикла длопена игры
//Если нужно что-то из жни бери g_env он инициализируется в Game чеке
//И да если будешь делать хост то рил http request не юзай
...
static float g = 0.0f;
static float b = 0.0f;
static float ro = 255.0f;
static float go = 0.0f;
static float bo = 0.0f;
static float rg = 255.0f;
static float gg = 0.0f;
static float bg = 0.0f;
static float rw = 255.0f;
static float gw = 0.0f;
static float bw = 0.0f;
static float rs = 255.0f;
static float gs = 0.0f;
static float bs = 0.0f;
static int w = 2;
static float gwidtch = 5.0f;
static float ow = 3.0f;
static float alphach = 255;
static float a = alphach / 255;
static int pulsesw = 1, pulsevalue = 9;
static float nightval = 1;
...
void SetRainbowChams(bool enable){
    enableRainbow1 = enable;
}

void SetDarkValue(int set){
    nightval = set;
}
...
void new_glDrawElements(GLenum mode, GLsizei count, GLenum type, const void *indices) {
    {
        GLint currProgram;
        glGetIntegerv(GL_CURRENT_PROGRAM, &currProgram);
        GLint id = old_glGetUniformLocation(currProgram, S("unity_SHC"));
        GLint id2 = old_glGetUniformLocation(currProgram, S("_Color"));
        GLint id22 = old_glGetUniformLocation(currProgram, getShader());
        GLint id3= old_glGetUniformLocation(currProgram, S("_MainTex"));
        if (id2 != -1 && id == -1) {
            if (mode != GL_TRIANGLES || count < 1) return;
        }

        if (id22 == -1){ old_glDrawElements(mode, count, type, indices); return;}
        if (!removetexture) old_glDrawElements(mode, count, type, indices);
        if (getWireframeEnabled()) {
            ...
            glBlendColor(GLfloat((rw / 255) * (float(pulsevalue) /100.0f)), GLfloat((gw / 255) * (float(pulsevalue) /100.0f)), GLfloat((bw / 255) * (float(pulsevalue) /100.0f)), a);
            glColorMask(1, 1, 1, 1);
            glEnable(GL_BLEND);
        ...
        if (getWallhackEnabled()) {
            glDepthRangef(1, 0.5);
            glBlendColor(GLfloat((r / 255) * (float(pulsevalue) /100.0f)), GLfloat((g / 255) * (float(pulsevalue) /100.0f)), GLfloat((b / 255) * (float(pulsevalue) /100.0f)), a);
        ...
        if (getShadingEnabled()) {
            ...
            glBlendColor(GLfloat((rs / 255) * (float(pulsevalue) /100.0f)), GLfloat((gs / 255) * (float(pulsevalue) /100.0f)), GLfloat((bs / 255) * (float(pulsevalue) /100.0f)), a);
        ...
        if (getGlowEnabled()) {
            glEnable(GL_BLEND);
            glBlendColor(GLfloat((rg / 255) * (float(pulsevalue) /100.0f)), GLfloat((gg / 255) * (float(pulsevalue) /100.0f)), GLfloat((bg / 255) * (float(pulsevalue) /100.0f)), a);
            glColorMask(1, 1, 1, 1);
            glEnable(GL_BLEND);
            glBlendFuncSeparate(GL_CONSTANT_COLOR, GL_CONSTANT_ALPHA, GL_ONE, GL_ZERO);
            glLineWidth(gwidtch);
        ...
        if (getOutlineEnabled()) {
            ...
            old_glDrawElements(GL_TRIANGLES, count, type, indices);
            glBlendColor(GLfloat((ro / 255) * (float(pulsevalue) /100.0f)), GLfloat((go / 255) * (float(pulsevalue) /100.0f)), GLfloat((bo / 255) * (float(pulsevalue) /100.0f)), a);
            glDepthRangef(1, 0.5);
            glBlendColor(GLfloat((ro / 255) * (float(pulsevalue) /100.0f)), GLfloat((go / 255) * (float(pulsevalue) /100.0f)), GLfloat((bo / 255) * (float(pulsevalue) /100.0f)), a);
            old_glDrawElements(GL_LINES, count, type, indices);
        }
...
bool mlovinit(){
    if(!gl2_handle){
        LOGD("Cockedddddddd ohhhhhhhhhhhhh ooooooooomwmh");
        return false;
    }
    return true;
}
...
void LogShaders(){
    ...
    auto p_glGetUniformLocation = (const void*(*)(...))dlsym(gl2_handle, S("glGetUniformLocation"));
    const char *dlsym_error = dlerror();
    if(dlsym_error){
        LOGD("Cannot load symbol 'glGetUniformLocation': %s", dlsym_error);
        return;
    }else{
...
void Wallhack(){
    ...
    auto p_glDrawElements = (const void*(*)(...))dlsym(gl2_handle, S("glDrawElements"));
    const char *dlsym_error = dlerror();
    if(dlsym_error){
        LOGD(S("Cannot load symbol 'Mmwmhawhahahmwh meffeddrrroon': %s"), dlsym_error);
        return;
    }else{
